...
export function studentRouterFactory(studentService: StudentService, classService: ClassService) {
  ...
  router.get('/', async (_, res) => {
    return res.json(((await studentService.list()) as Student[]).map((student: Student) => student.toObject())) // FIXME: Como melhorar?
  })
  ...
  router.get('/:id/parents', async (req, res, next) => {
    try {
      const { id } = req.params
      const parents = (await studentService.getParents(id)) as Parent[] // FIXME: Como melhorar?
  ...
  router.patch(
    ...
    async (req: Request<{ id: string }, any, Pick<StudentCreationType, 'parents'>>, res, next) => {
      try {
        ...
        return res.json((await studentService.linkParents(id, parents)).toObject())
      } catch (error) {
        next(error)
      }
    }
