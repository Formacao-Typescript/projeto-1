...
export function studentRouterFactory(studentService: StudentService, classService: ClassService) {
  ...
  router.set('PUT /students/:id', async (req, res) => {
  ...
  router.set('DELETE /students/:id', async (req, res) => {
    studentService.remove(req.params.id)
    return void res.status(204).end()
  })

  router.set('GET /students/:id/parents', async (req, res) => {
    const { id } = req.params
  ...
  router.set('PATCH /students/:id/parents', async (req, res) => {
